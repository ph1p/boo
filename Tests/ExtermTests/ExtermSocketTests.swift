...
// MARK: - ExtermSocketServer Unit Tests

@MainActor
final class ExtermSocketServerTests: XCTestCase {

    func testSocketPathInExtermDir() {
        let server = ExtermSocketServer.shared
        XCTAssertTrue(server.socketPath.hasPrefix(ExtermPaths.configDir))
    ...
    func testDirectProcessRegistration() {
        ...
        server.processes[pid] = ExtermSocketServer.ProcessStatus(
    ...
    func testActiveProcessFindsDescendant() {
        ...
        server.processes[myPid] = ExtermSocketServer.ProcessStatus(
    ...
    func testMultipleProcesses() {
        ...
        server.processes[pid1] = ExtermSocketServer.ProcessStatus(
            pid: pid1, name: "claude", category: "ai", registeredAt: Date(), metadata: [:])
        server.processes[pid1 + 1] = ExtermSocketServer.ProcessStatus(
...
@MainActor
final class ExtermSocketProtocolTests: XCTestCase {
    ...
    func testSetAndClearStatus() {
        ...
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.name, "claude")
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.category, "ai")
        ...
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertNil(ExtermSocketServer.shared.processes[pid])
    }
    ...
    func testSetStatusWithMetadata() {
        ...
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.metadata["suite"], "e2e")
    ...
    func testDefaultCategoryIsUnknown() {
        ...
        _ = sendCommand("""
            ...
            """)
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.category, "unknown")
...
@MainActor
final class ExtermSocketBridgeTests: XCTestCase {
...
@MainActor
final class ExtermSocketPluginHandlerTests: XCTestCase {
    ...
    func testCustomPluginHandler() {
        ...
        ExtermSocketServer.shared.registerHandler(namespace: "myplugin") { json in
            received = json
    ...
    func testNamespacedCommandRouting() {
        ...
        ExtermSocketServer.shared.registerHandler(namespace: "git") { _ in
            gitHandled = true
            return ["ok": true]
        }
        ExtermSocketServer.shared.registerHandler(namespace: "docker") { _ in
            dockerHandled = true
            return ["ok": true]
        }
    ...
    func testMultipleCategories() {
        ...
        _ = sendCommand("""
            ...
            """)
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.category, "test")
        ...
        _ = sendCommand("""
            ...
            """)
        Thread.sleep(forTimeInterval: 0.2)
        XCTAssertEqual(ExtermSocketServer.shared.processes[pid]?.category, "build")
    ...
    func testMultipleProcessesDifferentCategories() {
        ...
        ExtermSocketServer.shared.processes[pid1] = ExtermSocketServer.ProcessStatus(
            pid: pid1, name: "claude", category: "ai", registeredAt: Date(), metadata: [:])
        ExtermSocketServer.shared.processes[pid1 + 1] = ExtermSocketServer.ProcessStatus(
            pid: pid1 + 1, name: "webpack", category: "build", registeredAt: Date(), metadata: [:])

        // Filter by category
        let ai = ExtermSocketServer.shared.activeProcess(shellPID: parentPid, category: "ai")
        XCTAssertEqual(ai?.name, "claude")

        // Build category PID is likely not a real descendant, so may be nil
        let any = ExtermSocketServer.shared.activeProcess(shellPID: parentPid)
...
@MainActor
final class ExtermSocketFullE2ETests: XCTestCase {
