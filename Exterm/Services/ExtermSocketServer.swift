...
/// Generic Unix socket server for IPC between terminal child processes and Exterm.
...
final class ExtermSocketServer {
    static let shared = ExtermSocketServer()
    ...
    /// Socket path: ~/.exterm/exterm.sock
    ...
    func start() {
        queue.async { [self] in
            ...
            guard serverFD >= 0 else {
                NSLog("[ExtermSocket] Failed to create socket: \(errno)")
                return
            }
            ...
            guard bindResult == 0 else {
                NSLog("[ExtermSocket] bind failed: \(errno)")
                close(serverFD)
                serverFD = -1
                return
            }
            ...
            guard listen(serverFD, 8) == 0 else {
                NSLog("[ExtermSocket] listen failed: \(errno)")
                close(serverFD)
                serverFD = -1
                return
            }

            NSLog("[ExtermSocket] Listening on \(socketPath)")
    ...
    private func handleSetStatus(json: [String: Any], clientFD: Int32) {
        ...
        _processes[p] = status
        ancestorCache.removeAll()
        NSLog("[ExtermSocket] Status set: pid=\(pid) name=\(name) category=\(category)")
    ...
    private func handleClearStatus(json: [String: Any], clientFD: Int32) {
        ...
        if _processes.removeValue(forKey: p) != nil {
            ancestorCache.removeAll()
            NSLog("[ExtermSocket] Status cleared: pid=\(pid)")
            notifyChanged()
        }
        sendResponse(fd: clientFD, ok: true)
    }
    ...
    private func sweepDeadProcesses() {
        ...
        for (pid, status) in _processes {
            ...
            if kill(pid, 0) == -1, errno == ESRCH {
                NSLog("[ExtermSocket] Dead process: pid=\(pid) name=\(status.name)")
