...
/// Manages per-session background SSH master connections with Exterm-owned sockets.
...
final class SSHControlManager {
    ...
    private struct ManagedConnection {
        ...
        /// Whether Exterm spawned the master (vs reusing user's existing ControlMaster).
        var isManaged: Bool
    }

    private var connections: [String: ManagedConnection] = [:]
    private let queue = DispatchQueue(label: "com.exterm.ssh-control", qos: .utility)
    ...
    static func socketFilePath(for alias: String) -> String {
        (ExtermPaths.sshSocketsDir as NSString).appendingPathComponent("exterm-cm-\(sanitize(alias))")
    }
    ...
    private func cleanStaleSockets() {
        let dir = ExtermPaths.sshSocketsDir
        guard let files = try? FileManager.default.contentsOfDirectory(atPath: dir) else { return }
        for file in files where file.hasPrefix("exterm-cm-") {
