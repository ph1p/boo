...
final class RemoteExplorer: @unchecked Sendable {
    ...
    private static func findChildProcess(parentPID: pid_t, name: String, maxDepth: Int = 4) -> pid_t? {
        let children = ProcessTree.childPIDs(of: parentPID)
        for child in children {
            ...
            if procName.isEmpty, let args = ProcessTree.processArgs(pid: child) {
    ...
    private static func parseSSHHost(pid: pid_t) -> String? {
        guard let cmdline = ProcessTree.processArgs(pid: pid) else { return nil }
    ...
    static func isSSHTunnel(pid: pid_t) -> Bool {
        guard let cmdline = ProcessTree.processArgs(pid: pid) else { return false }
        let args = cmdline.trimmingCharacters(in: .whitespacesAndNewlines).split(separator: " ").map(String.init)
